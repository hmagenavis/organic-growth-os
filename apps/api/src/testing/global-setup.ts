...
/**
 * Provides a real PostgreSQL server for the API integration suite.
 *
 * Deliberately a copy of `packages/database/src/testing/global-setup.ts` rather than an
 * import: Testcontainers is a devDependency of each package that needs it, and making
 * the database package export test infrastructure would put `vitest/node` types into
 * its published surface. The duplication is thirty lines of test plumbing and both
